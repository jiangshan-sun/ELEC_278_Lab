...
void bst_intersection_helper(bst tree1, bst tree2, bst *intersection_tree) {
    if (tree1 != NULL && tree2 != NULL) {
        if (tree1->value == tree2->value) {
            // Insert all occurrences of the common value into the intersection tree
            bst_intersection_helper(tree1->left, tree2->left, intersection_tree);
            bst_insert(intersection_tree, tree1->value);
            bst_intersection_helper(tree1->right, tree2->right, intersection_tree);
        } else if (tree1->value < tree2->value) {
            bst_intersection_helper(tree1->right, tree2, intersection_tree);
        } else {
